...
$(pGreen '*)') PHP with Essential Extensions (from 8.2 to 8.0)
...
$(pBlue "Are you sure you want to continue? Type $(pGreen 'Y') to Continue or $(pRed 'N') to Abort"): "
	...
	n | N)
		_bold "Aborted, Going to Main Menu"
		exit 0
		;;
	*)
		_error "Wrong Choice !!, Y for Continue, N for Abort"
		menu
		;;
	esac
}
...
function continue_install() {
	...
	if id "$user_reloader" &>/dev/null; then
